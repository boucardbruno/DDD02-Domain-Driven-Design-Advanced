...
public class AuditoriumLayoutRepository
{
    ...
    private static string GetExecutingAssemblyDirectoryFullPath()
    {
        ...
        if (directoryName != null && directoryName.StartsWith(@"file:\")) directoryName = directoryName.Substring(6);

        if (directoryName != null && directoryName.StartsWith(@"file:/")) directoryName = directoryName.Substring(5);
